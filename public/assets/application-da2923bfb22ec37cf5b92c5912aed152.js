...
(function() {
  $(document).ready(function() {
    ...
    return $("#new_upload").dropzone({
      ...
      complete: function(file, XMLHttpRequest, xhr) {
  ...
  jQuery(function() {
    ...
    length_check = function(target, min, max) {
      ...
      if (($(target).val() != null) && ($(target).val() != null)) {
        if (_ref = $(target).val().length, __indexOf.call((function() {
          _results = [];
        ...
        if (_ref1 = $(target).val().length, __indexOf.call((function() {
          _results1 = [];
          for (var _j = min; min <= max ? _j <= max : _j >= max; min <= max ? _j++ : _j--){ _results1.push(_j); }
          return _results1;
        }).apply(this), _ref1) >= 0) {
          return $(target).parent().removeClass('has-error').addClass('has-success');
        }
      }
    };
    ...
    description_label = function(description_remaining) {
      if ($('label#description_feedback').html != null) {
        return $('label#description_feedback').html('Description' + ' (' + description_remaining + ' characters remaining)');
      }
    };
    ...
    $(emails).keyup(function() {
      if (email_check($(emails).val())) {
        return length_check(emails, 5, 30);
      } else {
        return length_check(emails, 998, 999);
      }
    });
    $(passwords).keyup(function() {
      return length_check(passwords, 8, 128);
    });
    $('#current_password').keyup(function() {
      return length_check('#current_password', 8, 128);
    });
    return $(password_confirms).keyup(function() {
      if ($(password_confirms).val() === $(passwords).val()) {
        return length_check(password_confirms, 8, 128);
      } else {
        return length_check(password_confirms, 998, 999);
      }
    });
  });
